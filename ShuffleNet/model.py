...
class InvertedResidual(nn.Module):
    def __init__(self, input_c: int, output_c: int, stride: int):
        ...
        # branch 2(the right branch in Fig3(c and d))
    ...
    def forward(self, x: Tensor) -> Tensor:
        if self.stride == 1: # Fig 3(c)
            x1, x2 = x.chunk(2, dim=1) # use .chunk to split the channel, which is dim=1
            out = torch.cat((x1, self.branch2(x2)), dim=1) # x2 goes through the right branch in Fig 3(c) and then concat with x1 on channel dim
        else: # Fig 3(d)
            out = torch.cat((self.branch1(x), self.branch2(x)), dim=1) # x1 goes through the left channel in Fig 3(d); x2 goes through the right channel in Fig 3(d); then concat 
        out = channel_shuffle(out, 2)# do channel shuffle after concatenation
        return out

class ShuffleNetV2(nn.Module):
    def __init__(self,
                 stages_repeats: List[int],# [4,8,4]
                 stages_out_channels: List[int],# out_channel for different shufflenet version; length is 5(stage 1 to 4 and conv5)
                 num_classes: int = 10,
        ...
        # repeat stage from Stage 2 to Stage 4
