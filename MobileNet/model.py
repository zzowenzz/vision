...
# ———————————————————————————————————————————— MobileNetV3————————————————————————————————————————————
class ConvBNActivation(nn.Sequential): # conv2d + norm_layer + ac
    def __init__(self,
                 in_planes: int,
                 out_planes: int,
                 kernel_size: int = 3,
                 stride: int = 1,
                 groups: int = 1,
                 norm_layer: Optional[Callable[..., nn.Module]] = None,  # norm_layer can be nn.Module or None
                 activation_layer: Optional[Callable[..., nn.Module]] = None): # activation_layer can be nn.Module or None
...
class SqueezeExcitation(nn.Module): # avgpool2d + down dim + up dim + hardsigmoid
...
class InvertedResidualConfig: # configuration file for inverted residual
    def __init__(self,
                 input_c: int,
                 kernel: int,
                 expanded_c: int,
    ...
    @staticmethod  # bound to a class rather than the objects for that class
    def adjust_channels(channels: int, width_multi: float):
        return _make_divisible(channels * width_multi, 8)

class InvertedResidualV3(nn.Module):
    def __init__(self,
                 cnf: InvertedResidualConfig,  # @staticmethod in InvertedResidualConfig
...
class MobileNetV3(nn.Module):
    def __init__(self,
        ...
        layers.append(ConvBNActivation(1,  # in_channel set to 1 for FashinMINIST dataset
                                       firstconv_output_c,
                                       kernel_size=3,
                                       stride=2,
                                       norm_layer=norm_layer,
...
def mobilenet_v3_large(num_classes: int = 10,
...
def mobilenet_v3_small(num_classes: int = 10,
                       reduced_tail: bool = False) -> MobileNetV3:
    width_multi = 1.0
    bneck_conf = partial(InvertedResidualConfig, width_multi=width_multi)
    adjust_channels = partial(InvertedResidualConfig.adjust_channels, width_multi=width_multi)
...
# with open("MobileNetV3_S.txt", "w") as f:
